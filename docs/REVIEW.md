...
| SYNC-2 | `registry/profiles.py:13` | Default seed copies `settings.json`+`plugins/` verbatim; the path-based denylist does not field-patch settings.json inbound, so the host `SessionEnd → sync-claude.sh --save` hook + bun statusLine would be seeded into the container (a forbidden host-tool co-fire). Not live (no seeder yet). | Inbound field-patch to strip `hooks`/`statusLine`; trim `plugins/` (exclude cache/data/blocklist.json). | PLANNED (Phase 2) |
