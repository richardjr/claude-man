...
| 5 | **usage** | HARD | `usage.py` (transcript JSONL schema), `usage_api.py` (`/api/oauth/usage`, beta header, scopes) | the transcript location + schema and the subscription-usage endpoint — or `usage=None` to drop the feature (render/level helpers are neutral) |
