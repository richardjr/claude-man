...
2. **The hardened run profile is the floor, not a suggestion.** `--read-only`, `--cap-drop ALL`,
   ...
   **opt-in** writable surface exists: **only when `config shell-history on`** (default OFF), a
   ...
   And the container's **`TMPDIR` is redirected to `/workspace/.tmp`** (issue #42): Node's
   `os.tmpdir()` / python `tempfile` / `mktemp` default to `/tmp`, the 512m tmpfs, and Yarn Berry
   zip-converts every fetched tarball there in parallel workers — a heavy install overflowed it with a
   misleading ENOSPC on every host OS. The dir is a wiped-each-session subdir of the bind that
   `lifecycle` (re)creates on start/stop via `scratch.clear_tmp` (nothing that honours `TMPDIR`
   creates it); `TMUX_TMPDIR=/tmp` pins the status bar's tmux socket BACK onto the tmpfs, since a
   unix socket on a Docker Desktop virtiofs bind is unreliable. Both are `_BAKED_ENV` redirects — the
   `/tmp` tmpfs line and the floor are byte-identical, unit-pinned, and `image smoke` gates both.)
...
6. **One `claude` per container.** A second shell is fine; a second `claude` in the same container
   ...
   with ONE amendment (issue #37): a claude living inside the status bar's tmux `claude` session
   (`claude_session_exists`, fails CLOSED → refuse) is RE-ATTACHED (`claude-man-bar`'s
   `new-session -A`) rather than refused, since closing its window never killed it; a claude running
   anywhere else in the container is still refused.
...
src/claudeman/
  ...
  agents/              the agent-provider seam (Phase 7a — docs/AGENTS.md + docs/V2-PLAN.md): base.py (PURE frozen `AgentProvider` + `AuthSpec`/`ImageSpec`/`UpdateSpec` value objects, validated so no provider field can carry shell into the probe or argv), claude.py (the reference provider — values sourced from config so paths/env names keep one home), `resolve(id)` (the ONE way to reach provider policy; `binaries()`/`config_dirs()`/`config_dir_envs()` derive the cross-provider sets the guards use). Routed today: the spawn binary + /proc comm probe (terminals), the config-dir path + env pointer + token env + scrub set (runner `_baked_env`/`_is_scrubbed`, setup_token verify), the image version build-arg + label key (images/labels), the release pointer (updates), `required_hosts` → `allowlist.base_allowlist(provider)` (+ the neutral `TOOLCHAIN`), and the mount-dst denylist / tool reserved-env over EVERY provider's config dir. A provider parameterises BEHAVIOUR never SECURITY — the floor is byte-identical per provider (tests/test_agents.py pins the pre-seam goldens). Still hard-coded until 7a-2/7-auth/7-run: sync-back policy data, the CLAUDE.md context spec + packs, the smoke probes, the host mint flow. **7b:** `Project.agent` + `Profile.agent` (default `claude`, validated against `agents.ids()`; `Project.provider` resolves it), the `claude-man.agent` label + `status.Row.agent` (registry wins) + the AGENT column (CLI `project status` / TUI table) + `project create --agent` / the create modal's Agent select; `lifecycle.agent_mismatch` refuses a profile of another provider; `terminals.provider_for(slug)` (registry, fail-open claude) feeds spawn/probe; lifecycle/egress/images take the project's provider
  ...
  scratch.py           per-session scratch / data-transfer dir (`/workspace/scratch`): a SUBDIR of the existing /workspace bind (NOT a new mount — hardened floor byte-identical, invariant 2). `clear(slug)` containment-checked wipe+recreate on every start AND stop (best-effort; never blocks start/stop); `ensure_note(project)` stamps a managed block into /workspace/CLAUDE.md telling the agent to look there for "provided files" (via claudemd, in-place, operator content preserved). Wiped-each-session, so durable work goes in a repo under /workspace; a repo dir under `scratch/` is refused at `project repo add`. `clear_tmp(slug)` is the same wipe+recreate for the per-session temp dir `/workspace/.tmp` — the container's `TMPDIR` (issue #42: temp on the disk-backed bind, not the 512m /tmp tmpfs; must exist before start, so the lifecycle creates it here host-side); a repo dir under `.tmp/` is refused likewise
  statusbar.py         PURE renderer of the per-project tmux STATUS BAR strings (issue #37 — the top row of every claude/shell window: slug chip in `config.project_name_color` on the `project_tint` background + profile/auth/image/model/egress + a fixed git-branch probe/clock) → `BarSpec` + its `CLAUDE_MAN_BAR_{STYLE,LEFT,RIGHT}` exec-time env; every operator value `##`-escaped so no registry string can smuggle a tmux `#()` shell into the bar. `terminals.bar_spec` does the registry read (fail-open to a slug-only bar); the in-container half is images/bash/{tmux.conf,claude-man-bar}. Launch-time, `config status-bar` (default on), floor byte-identical
  ...
  doctor.py            host prerequisite checks behind `claudemanctl doctor` + the TUI setup wizard/startup banner: PURE classify_* verdicts (docker distinguishes binary-missing / daemon-down / socket-EACCES / accepted-but-silent with per-cause fix hints; claude/image/profiles WARN not FAIL) + never-raising probe_* shells (the updates.py pattern) + run_all() -> Report. `probe_docker` retries ONCE on a first-attempt timeout with a 30 s cold-start budget (issue #34): a timeout is never "daemon down" (that is a fast `Cannot connect` rc 1) but a socket-activated dockerd being started for its first client — on a fresh boot, the TUI's own probe — which takes ~8 s to `API listen`; a lone 6 s attempt false-FAILed. Only a daemon silent for the whole budget is reported hung (status/restart hint, never `start`). No textual imports
  ...
  docker/              labels.py (incl. the `claude-man.auth` mode label — a login container self-describes, invariant 4), runner.py (hardened `docker create` argv + the ALWAYS-present `_render_memory` hard cap (`--memory X --memory-swap X`, value from settings) + env_file scrub + additive env-mount render + exec-stdin ssh seed + git_env identity + baked GIT_CONFIG_GLOBAL/GH_CONFIG_DIR redirects), status.py (live ps JOIN), stats.py (per-container `docker stats` NetIO — pure argv+parse, time-bounded wrapper — the TUI Network panel's Traffic figure), images.py (build/exists + base→overlay→tools-layer auto-build chain; `ensure_chain` (the up/create/recreate pre-flight) builds MISSING *and STALE* layers — a layer whose `.Created` predates its parent's (`layer_stale` over the pure `parse_created`/`is_stale`: the parent was rebuilt since, e.g. `image build base` after a base change; a cache-hit rebuild keeps the original Created so an unchanged base never falsely stales) is rebuilt and cascades down the chain; `cmd_image_build` applies the same check to the requested image's parents, so `image build --project` after a base rebuild rebuilds the stale overlay first — the issue #37 'no difference' case), smoke.py (hardened-profile image gate)
  ...
  tools/               the approved-tool registry (Phase 10 — docs/TOOLS.md): library.py (PURE discovery/parse/validate of `library/tools/<name>/tool.toml` — `kind = "apt"` packages or `kind = "release"` per-arch `url`+`sha256` artefacts (binary/tar/deb/bundle — `bundle` = a zip's self-contained `tree` installed whole to `/opt/<name>` + its `bins` symlinked into /usr/local/bin, the AWS CLI v2 shape; needs `unzip` in `build_deps`), `[env]` read-only-floor redirects (reserved/forbidden keys rejected — invariant 1), `[[smoke]]` core-op probes, `requires` closure (`resolve`, acyclic), `build_deps`, a locked-egress `allowlist` hint; the shipped registry is lint-tested), render.py (PURE Dockerfile renderer — ONE layer `FROM claude-man:<overlay>`, apt in one RUN, each release artefact `sha256sum -c`-verified, build deps purged, the tools' env as overlay-scoped ENV, `claude-man.tools` label — + the content-addressed image name `<overlay>-t-<sha256[:12] of the render>` (`config.tools_image_overlay` is the inverse) + `materialize` writing it to the state tier `config.tools_dockerfile_path`). Selection = `Project.tools` (replaced the never-implemented `extra_apt` stub); ADDITIVE on top of ANY overlay (no new overlay type — "custom" = base + a selection); `lifecycle.resolve_image` renders it, `images.build_chain` grows a third step (base → overlay → layer, so the on-start claude update rebuilds it too), the runner takes the tag via `build_create_argv(image=…)` (only the image token changes — floor byte-identical, unit-pinned), `smoke._tool_probes` appends every selected tool's registry probes (`image smoke --project`). Recreate-to-apply (`lifecycle.set_tools` is registry-only + validated against the registry first + the locked-egress host hint), like ports/env-mounts. Superseded `-t-` images are not auto-pruned (docs/TOOLS.md follow-ups)
...
library/tools/         the approved-tool registry (`<name>/tool.toml` — docs/TOOLS.md): kubectl, helm (v3), k9s (requires kubectl), aws-cli (the v2 bundle — the terraform overlay's artefact, now selectable on any overlay), session-manager-plugin, postgresql-client, jq, python3, python3-yaml, uv — every entry pinned (+ per-arch sha256 for release artefacts, verified at build) with its floor env redirects (kubeconfig/helm config/pgpass/aws config+creds/k9s config → the `.cache` tmpfs since they hold credentials or session state; helm data+cache → /workspace; aws-cli also sets `AWS_PAGER=""` — no `less` in the image, issue #41, pinned by a `script`-faked-TTY probe) + smoke probes. PUBLIC content — house tools in
...
tests/                 dependency-free unittest suite (argv renderer, env-file scrub, denylist, registry, seed, usage, smoke verdict, tool registry lint + render)
```
...
- **CLI first, then TUI — every operator-facing feature ships in BOTH, in that order.** The
  behaviour lives once in the backend (`lifecycle.py` + the registry/pure modules, importable
  without textual and unit-tested dependency-free); `cli.py` exposes it as `claudemanctl` verbs
  FIRST (the CLI is the scriptable, testable contract and the reference for the docs); then the
  TUI gets its parity surface in the SAME change — a screen/modal (or a field on an existing one,
  e.g. the New-project modal) plus a Project…/Repos…/View… menu row or Settings key, wired to the
  same lifecycle call off-thread via a `@work` worker and `_reserve`/`_after_action`. A feature
  is not done while it is CLI-only (issue #31 was exactly that gap — an unlisted terminal was
  CLI-only), and the TUI must never grow logic the CLI can't reach (no TUI-only code paths into
  the registry or docker; the pure view model — `tui/*view.py`, textual-free, unit-tested — is
  the only TUI-side logic). Both surfaces get documented: the verb in `docs/CLI.md` + the key
  bar/menu row in `docs/TUI-GUIDE.md`, and the CLAUDE.md layout + verbs lists above.
...
tools list [-v]                                     # browse the approved-tool registry (library/tools/ — docs/TOOLS.md); -v adds env redirects + locked-egress hosts
project tools add|rm <slug> <name> | tools list <slug>   # select/deselect an approved tool — validated against the registry NOW, baked as a content-addressed image layer on top of the overlay at the next recreate/up (recreate-to-apply; TUI: Project menu -> Tools (image)… `t`, Apply recreates). create flag: `--tool NAME` (repeatable)
image build|smoke --project <slug>                  # build / hardened-smoke the image a project runs on (overlay + its tools; parents built first; smoke adds every selected tool's core-op probes)
...
config status-bar [on|off]                         # the per-project tmux STATUS BAR on the top row of spawned claude/shell windows (slug in its colour + profile/auth/image/model/egress + git branch; default on; launch-time, no recreate; image must bake the launcher — a stale image opens plainly). TUI: Settings (,) -> s
