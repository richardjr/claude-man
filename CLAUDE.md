...
library/tools/         the approved-tool registry (`<name>/tool.toml` — docs/TOOLS.md): kubectl, helm (v3), session-manager-plugin, postgresql-client, jq, python3, python3-yaml, uv — every entry pinned (+ per-arch sha256 for release artefacts, verified at build) with its floor env redirects (kubeconfig/helm config/pgpass → the `.cache` tmpfs since they hold credentials; helm data+cache → /workspace) + smoke probes. PUBLIC content — house tools in
