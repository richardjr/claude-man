...
def _apply_locked(slug: str, decisions: dict[str, str]) -> MergeReport:
    ...
    for c in accepted:
        if c.kind in ("tree", "tree-symlink", "file"):
            ok, audit_rel, host_path, deleted, reason, sub = _apply_tree_change(slug, c, ts, policy)
            notes.extend(sub)
            if ok:
    ...
    if settings_changes:
        ...
        ok, host_path, applied_keys, sub = _apply_settings(slug, settings_changes, ts, policy)
        notes.extend(sub)
        if ok:
            applied.extend(f"{settings}/{k}" for k in applied_keys)
            if applied_keys:
                staged.append((settings, host_path, False))
        else:
            refused.extend(f"{settings}/{c.rel}: backup failed — not written" for c in settings_changes)

    audit_commit = _audit_commit(slug, staged, ts, policy) if staged else ""
...
def _apply_tree_change(slug: str, c: "detect.Change", ts: str, policy=None
    ...
    art = _artifact(c.artifact, policy)
    if art is None:
    ...
    if src.is_symlink():
        note = fsmerge.check_symlink(src, rel, src_root=container_root, gate=True, policy=policy)
        if note:
...
def _apply_settings(slug: str, changes: list["detect.Change"], ts: str, policy=None
    ...
    applied_keys, notes)``."""
    settings = policy.settings_file if policy is not None else "settings.json"
    art = _artifact(settings, policy) if settings else None
    if art is None:
        return False, None, [], [f"{settings or 'settings'} artifact missing"]
    host_path = Path(art.host_target)
    container_path = config.claude_config_dir(slug) / settings
    ...
    for c in changes:
        key = c.rel
        if key in denylist.immune_keys(policy):
            notes.append(f"{settings}/{key}: host-structural key never overwritten")
            continue
        if denylist.is_denied_json_key(key, policy):
            notes.append(f"{settings}/{key}: denied key skipped")
            continue
        if c.change_type == "deleted":
            ...
            notes.append(f"{settings}/{key}: container value vanished")
    ...
    try:
        ...
        return False, host_path, [], notes + [f"{settings} write failed: {exc}"]
...
# ---------------------------------------------------------------------------
def _audit_commit(slug: str, staged: list[tuple[str, Path, bool]], ts: str, policy=None) -> str:
    ...
    try:
        ...
        for audit_rel, host_path, deleted in staged:
            if denylist.is_denied_path(audit_rel, policy):  # staging-time re-assert (invariant 5)
                continue
