...
def gateway_config_yaml(model: str, *, ollama_host: str) -> str:
    ...
    lines = [
        ...
        "      # Force-drop the Anthropic `thinking` block on the LOCAL route only: Claude Code sends it at",
        "      # higher reasoning effort, but most local coder models (qwen3-coder, devstral, …) are NOT",
        "      # thinking models and Ollama hard-errors `does not support thinking`. drop_params alone misses",
        "      # it (LiteLLM's ollama provider generically `supports` think). The claude-* route above keeps",
        "      # thinking (Anthropic supports it). (issue #14)",
