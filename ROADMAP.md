...
- [x] **7a:** *(7a-1 LANDED 2026-09-26 — `agents/` + the claude provider; spawn/comm, config-dir+env, token env+scrub, version build-arg+label, release pointer, required hosts + the `TOOLCHAIN` split, and the cross-provider mount-dst/reserved-env guards routed; goldens pinned in `tests/test_agents.py`. 7a-2 LANDED with 7d — the sync-back policy data + the context/packs spec on the provider.)* introduce `agents/` + a `claude` provider reproducing today's behaviour byte-for-byte (pure refactor, zero behaviour change); route the soft seams through it (spawn binary/comm, config-dir path+env, version-label key+build-arg, release URL/UA, required egress hosts, context-file name + import syntax, sync-back policy data). A unit test pins the hardened argv byte-identical (invariant 2)
...
- [x] **7c:** *(LANDED 2026-09-26 — `agents/codex.py` + `library/tools/codex/tool.toml`: the provider installs THROUGH the tool registry (a tar.gz bundle, `tree = "."`, `version_label`) and is pulled into its projects' image automatically via `ImageSpec.tools`; `config_seed` for the sandbox-off `config.toml`; `syncback=False` until 7d; `project agent` alias; `image smoke --project` provider-aware)* a `codex` provider + image ~~overlay~~ layer, validated against the hardened floor (`image smoke`); ~~resolve the auth-kind divergence~~ *(RESOLVED — the 2026-09-26 login-mode spike, `docs/AGENTS.md` § Codex: `auth.json` minted in-container by `codex login --device-auth`, self-refreshing, survives recreate = our `login` mode; `OPENAI_API_KEY` = `token` mode; install = the full `codex-package` tarball as a `bundle` (the shell tool needs `codex-code-mode-host`); seed `sandbox_mode = "danger-full-access"` since bwrap can't namespace under cap-drop ALL; JSONL fixtures in `tests/fixtures/codex/`)*. `project create --agent codex`
- [x] **7d:** *(LANDED 2026-09-26)* Codex sync-back policy (`agents/codex.py::SYNCBACK` — skills-only, drawn from the real tree, test-pinned) + the context-file seam (`ContextSpec`: `AGENTS.md` with pack fragments inlined; skills-only assets; provider-aware `[project.sync]` defaults); the engine takes a `SyncbackPolicy` per call (claude byte-identical). Verified live on `codex-demo` (baseline without `auth.json`; an authored skill merged into `~/.codex/skills`). Pre-existing finding, not 7d's: `stop` runs the asset sync-out BEFORE the pending-sync-back note, and detect subtracts a bind file byte-equal to the asset source as claude-man's own write — so an agent-authored skill is absorbed into the project asset tier at stop and never shows as pending for the host tier (claude too). Tracked as a follow-up
...
- [x] **7-run:** *(LANDED 2026-09-26)* the headless seam — `RunRequest`/`RunSpec`/`AgentEvent` (`agents/run.py`: claude stream-json AND codex exec JSONL parsers, pinned by real fixtures in `tests/fixtures/{claude,codex}/`), `lifecycle.run` (`docker exec -i`, prompt on stdin, one-agent guard, timeout), `project run <slug> <prompt|-> [--permission|--resume|--model|--timeout|--json]` + the TUI Project menu `u` Run prompt… modal; verified live (tool call as uid 1000). ACP as a single transport: deferred — the two JSONL parsers are small and pinned; revisit if a third provider lacks a JSON stream
...
**Status (issue #14, 2026-06-26) — the spike is validated and both legs work end-to-end.** One LiteLLM
sidecar serves both. The Claude-leg "404 model: opus-4-8" was a model-MAPPING bug, NOT auth: Claude Code
sends the FULL id `claude-opus-4-8`, and the `claude-* → anthropic/*` wildcard captured the suffix →
invalid `opus-4-8`. Fixed with the prefix-preserving `anthropic/claude-*`; **subscription preserved,
verified live** (200, served on the subscription lane — the OAuth rides LiteLLM's DEDICATED anthropic path,
not `forward_client_headers_to_llm_api`). Discovery was dropped for `ANTHROPIC_CUSTOM_MODEL_OPTION` (more
reliable); `thinking`/`reasoning_effort` are force-dropped on the LOCAL route only (non-thinking coders
